...
public class TheForestEvent extends ListenerAdapter {
    ...
    public void onGuildVoiceJoin(@NotNull GuildVoiceJoinEvent event)
    {
        new Thread(() -> {
            ...
            if (event.getChannelJoined().getId().equals("798442796662390814")) {
                ...
                int i;
                for (i = 1; found; i++) {
                    found = ChannelNames.theForest.contains(i);
                }
                ChannelNames.theForest.add(i - 1);
                name = "「\uD83D\uDD08」 #" + (i - 1) + " TheForest";
    ...
    public void onGuildVoiceMove(@NotNull GuildVoiceMoveEvent event) {
        new Thread(() -> {
            ...
            } else if (Objects.requireNonNull(event.getChannelLeft().getParent()).getId().equals("798442411746918480") && !event.getChannelLeft().getId().equals("798442796662390814")) {
                if (event.getChannelLeft().getMembers().isEmpty()) {
                    ChannelNames.theForest.removeElement(Integer.parseInt(event.getChannelLeft().getName().substring(6, 7)));
                    try {
                        event.getChannelLeft().delete().queue();
