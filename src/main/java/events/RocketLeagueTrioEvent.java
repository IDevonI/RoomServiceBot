...
public class RocketLeagueTrioEvent extends ListenerAdapter {
    ...
    public void onGuildVoiceJoin(@NotNull GuildVoiceJoinEvent event)
    {
        new Thread(() -> {
            ...
            if (event.getChannelJoined().getId().equals("797301874998116383")) {
                ...
                int i;
                for (i = 1; found; i++) {
                    found = ChannelNames.rocketLeagueTrio.contains(i);
                }
                ChannelNames.rocketLeagueTrio.add(i - 1);
                name = "「\uD83D\uDD08」 #" + (i - 1) + " Trio";
    ...
    public void onGuildVoiceMove(@NotNull GuildVoiceMoveEvent event) {
        new Thread(() -> {
            ...
            } else if (Objects.requireNonNull(event.getChannelLeft().getParent()).getId().equals("797299044475994152") && !event.getChannelLeft().getId().equals("797301874998116383")) {
                if (event.getChannelLeft().getMembers().isEmpty() && event.getChannelLeft().getName().contains("Trio") && !event.getChannelLeft().getName().contains("➕")) {
                    ChannelNames.rocketLeagueTrio.removeElement(Integer.parseInt(event.getChannelLeft().getName().substring(6, 7)));
                    try {
                        event.getChannelLeft().delete().queue();
